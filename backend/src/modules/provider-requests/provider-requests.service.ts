...
export class ProviderRequestsService {
  ...
  async generateSignedUrl(
    mimeType: string,
    filename: string,
  ): Promise<{ signedUrl: string; finalUrl: string }> {
  ...
  async marcarPagoYConvertir(solicitudId: number): Promise<void> {
    const solicitud = await this.requestRepo.findOne({
    ...
    if (solicitud.pagoRealizado)
      throw new BadRequestException('El pago ya fue registrado');

    solicitud.pagoRealizado = true;

    // Si deseas mantener la revisión manual, deja esta línea.
    // Si no hay revisión manual, puedes omitir esta validación.
    ...
    // Crear registro en tabla providers
  ...
  async findLatestByUser(userId: number): Promise<ProviderRequest | null> {
